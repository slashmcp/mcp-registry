...
/**
 * Get install configuration for a server and client
 */
export async function getInstallConfig(
  serverId: string,
  client: InstallClient
): Promise<InstallConfigResponse> {
  const encodedId = encodeURIComponent(serverId)
  const response = await fetch(`${API_BASE_URL}/v0.1/servers/${encodedId}/install/${client}`)
  
  if (!response.ok) {
    const error = await response.json().catch(() => ({ error: response.statusText }))
    throw new Error(error.message || error.error || `Failed to get install config: ${response.statusText}`)
  }
...
export async function getServerPermissions(serverId: string): Promise<ServerPermissionsResponse['permissions']> {
  const encodedId = encodeURIComponent(serverId)
  const response = await fetch(`${API_BASE_URL}/v0.1/servers/${encodedId}/permissions`)
  
  if (!response.ok) {
    const error = await response.json().catch(() => ({ error: response.statusText }))
    throw new Error(error.message || error.error || `Failed to get permissions: ${response.statusText}`)
  }
  
  const data: ServerPermissionsResponse = await response.json()
